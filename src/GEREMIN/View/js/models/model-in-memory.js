geremin.Employee = Backbone.Model.extend({
    ...
    sync: function(method, model, options) {
        if (method === "read") {
            geremin.store.findById(parseInt(this.id), function (data) {
...
geremin.EmployeeCollection = Backbone.Collection.extend({
    ...
    sync: function(method, model, options) {
        if (method === "read") {
            geremin.store.findByName(options.data.name, function (data) {
                options.success(data);
            });
        }
    }
...
geremin.ReportsCollection = Backbone.Collection.extend({

    model: geremin.Employee,

    sync: function(method, model, options) {
        if (method === "read") {
            geremin.store.findByManager(this.parent.id, function (data) {
                options.success(data);
            });
        }
    }
...
geremin.MemoryStore = function (successCallback, errorCallback) {
...
geremin.store = new geremin.MemoryStore();